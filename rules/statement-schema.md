...
subject_role: maintainer    # maintainer | funder | user | researcher | unspecified
...
A facet describes what a statement is about, never who said it: `subject_role`
is the role implicated by the assertion, not the speaker's own role, which is
unknowable once provenance is gone. A facet whose value cannot be read from the
statement's own words does not belong in the config, because by the time
anything is labeled there is no provenance left to fall back on.
...
person does (as distinct from the closed `subject_role` facet above), dates,
direct quotes, timestamps, submission ids, form row identifiers, arrival order,
source file, hashes, statement ids, lifecycle states. No field links two
statements to the same interview: the step that writes these records never
learns which contribution a bullet came from.
