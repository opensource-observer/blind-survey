...
def test_each_operator_skill_exists(name):
    assert Path(f"skills/{name}/SKILL.md").is_file()
...
def test_each_skill_is_reachable_from_dot_claude(name):
    ...
    assert link.resolve() == Path(f"skills/{name}").resolve()


def test_setup_covers_every_stage_it_owns():
    text = Path("skills/setup/SKILL.md").read_text(encoding="utf-8")
    for needed in (
        "survey.yaml",
        "tools/compile.py",
        "verify_form.py",
